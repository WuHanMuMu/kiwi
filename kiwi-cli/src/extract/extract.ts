...
import { baiduTranslateTexts } from '../translate'
 ...
 * @param {dirPath} 文件夹路径
 */
async function extractAll(dirPath?: string) {
  // if (!CONFIG.googleApiKey) {
  //   console.log('请配置googleApiKey');
  //   return;
  // }
  // if (!CONFIG.baiduApiKey) {
  //   console.log('请配置百度apikey')
  //   return
  // }
  const dir = dirPath || './';
  ...
  for (let j = 0; j < allTargetStrs.length; j++) {
    const item = allTargetStrs[j];
    ...
    //   return prev.concat(baiduTranslateTexts(transText, 'en_US'));
    // }, []);
    const translatePromises = [];
    for (let index = 0; index < targetStrs.length; index++) {
      const curr = targetStrs[index];
      const reg = /[^a-zA-Z\x00-\xff]+/g;
      const findText = curr.text.match(reg);
      const transText = findText ? findText.join('').slice(0, 4) : '中文符号';
      
      // const result = await baiduTranslateTexts(transText, 'en_US')
      const result = [{}]
      // console.log(transText, result);
      translatePromises.push(result)
    }
      ...
      .then(([...translateTexts]) => {
        const replaceableStrs = targetStrs.reduce((prev, curr, i) => {
          ...
          if (!virtualMemory[curr.text]) {
            ...
            console.log(`${curr.text} => ${transKey}`, curr);
